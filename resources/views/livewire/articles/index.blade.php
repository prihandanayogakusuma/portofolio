<div class="py-28 relative z-10">
    <div class="container mx-auto px-6 max-w-7xl">
        ...
        <div class="text-center mb-12">
            <h1 class="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white mb-4">Artikel & <span class="text-indigo-600 dark:text-indigo-500">Catatan IT</span></h1>
            <div class="w-20 h-1 bg-indigo-500 mx-auto rounded-full mb-4"></div>
            <p class="text-slate-600 dark:text-slate-400">Kumpulan dokumentasi konfigurasi server, jaringan, dan otomatisasi sistem.</p>
        </div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-12">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <button wire:click="$set('category', '')" 
                    class="px-4 py-2 rounded-full text-sm font-medium transition-all {{ $category === '' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    Semua
                </button>
                @foreach($categories ?? [] as $cat)
                <button wire:click="$set('category', '{{ $cat }}')" 
                    class="px-4 py-2 rounded-full text-sm font-medium transition-all {{ $category === $cat ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    {{ $cat }}
                </button>
                @endforeach
            </div>
            ...
            <div class="w-full md:w-80 relative">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari artikel atau catatan..." 
                    class="w-full bg-white dark:bg-slate-800/40 border border-slate-300 dark:border-slate-700 rounded-full px-5 py-3 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-sm dark:shadow-none">
                <div class="absolute right-4 top-3.5 text-slate-400 dark:text-slate-500">
                ...
                <div class="p-6 flex-1 flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            @if($article->category)
                            <span class="text-xs px-2.5 py-1 bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 rounded-full">{{ $article->category }}</span>
                            @endif
                        </div>
                        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-3 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ $article->title }}</h3>
